...
import (
	...
	_itemShop "github.com/Bannawat101/project-shop-api/pkg/itemShop/exception"
...
//How to run the test with Command Line
//go to the project root directory
//run the command: go test -v ./tests/buyingItem_test.go
//run go test -v ./tests to run all test files in the tests folder
//run go test -v -run TestItemBuyingSuccess ./tests to run only TestItemBuyingSuccess function
//run go test -v -coverprofile=coverage.out ./tests to generate coverage report
//run go tool cover -html=coverage.out to view coverage report in browser
...
func TestItemBuyingSuccess(t *testing.T) {
	...
	type args struct {
		...
		in       *_itemShopModel.BuyingReq    //สิ่งที่ส่งเข้าไปทดสอบ
		expected *_playerCoinModel.PlayerCoin //ผลลัพธ์ที่คาดหวัง
	}
